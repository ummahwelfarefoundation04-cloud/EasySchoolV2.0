...
const App: React.FC = () => {
  ...
  const handleFactoryReset = () => {
    if (window.confirm("🚨 CRITICAL WARNING: This will delete ALL stored data and reset the app. Confirm?")) {
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl shadow-lg p-6 text-white relative overflow-hidden group transition-all hover:shadow-xl">
                 <div className="relative z-10 flex flex-col h-full justify-between">
                   <div className="flex justify-between items-start">
                     ...
                     <span className="text-xs font-semibold bg-white/20 px-2.5 py-1 rounded-full text-blue-50 border border-white/10">Active</span>
                   </div>
                   <div className="mt-4">
               ...
               <div className="bg-white rounded-2xl shadow-md p-6 border border-slate-100 relative overflow-hidden group transition-all hover:shadow-lg hover:border-green-200">
                 ...
                 <div className="flex flex-col h-full justify-between relative z-10">
                   <div className="flex justify-between items-start">
                     <div className="p-2.5 bg-green-50 rounded-xl"><GraduationCap size={24} className="text-green-600" /></div>
                     {currentSession ? <span className="text-xs font-semibold bg-green-100 text-green-700 px-2.5 py-1 rounded-full">Active</span> : <span className="text-xs font-semibold bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full">Not Set</span>}
                   </div>
                   <div className="mt-4">
               ...
               <div className="bg-white rounded-2xl shadow-md p-6 border border-slate-100 relative overflow-hidden group transition-all hover:shadow-lg hover:border-purple-200">
                 ...
                 <div className="flex flex-col h-full justify-between relative z-10">
                   <div className="flex justify-between items-start">
                     <div className="p-2.5 bg-purple-50 rounded-xl"><UserPlus size={24} className="text-purple-600" /></div>
                   </div>
                   <div className="mt-4">
                     <h3 className="text-3xl font-bold text-slate-800">{students.filter(s => s.admissionSessionId === currentSession?.id).length}</h3>
                     <p className="text-slate-500 text-sm font-medium mt-1">New Admissions</p>
                   </div>
                 </div>
               </div>
               ...
               <div onClick={() => { setEditingStudent(null); setActiveTab('live'); }} className="bg-slate-800 rounded-2xl shadow-md p-6 border border-slate-700 relative overflow-hidden group cursor-pointer hover:bg-slate-750 transition-all hover:shadow-lg hover:-translate-y-1">
                 <div className="flex flex-col h-full justify-between relative z-10">
                   ...
                   <div className="mt-4">
                     <h3 className="text-xl font-bold text-white">AI Live Talk</h3>
                     <p className="text-slate-400 text-sm font-medium mt-1 group-hover:text-slate-300">Voice Assistant</p>
                   </div>
                 </div>
               </div>
             </div>
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-20 overflow-y-auto shadow-2xl">
        <div className="p-6 flex items-center gap-3 border-b border-slate-700">
          <div className="w-10 h-10 bg-white rounded-lg shadow-lg flex items-center justify-center overflow-hidden shrink-0">
             {schoolProfile.logoUrl ? <img src={schoolProfile.logoUrl} alt="Logo" className="w-full h-full object-contain" /> : <GraduationCap size={20} className="text-blue-600" />}
          </div>
          <h1 className="text-sm font-bold leading-tight line-clamp-2">{schoolProfile.name || 'Easy School'}</h1>
        </div>
